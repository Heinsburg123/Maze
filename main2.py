    ...
    for j in range(start,28):
        ...
        if(check(heap,code,seg)==True and prev!=i and pos!=i):
            ...
            if(ans.get(u)!=None and len(ans[u])==0):
                ans.pop(u)
        if(prev!=i and ((cross(vec(arr[start],cen),vec(vertice[prev],cen))>=0 and index[prev]>j) or (cross(vec(arr[start],cen),vec(vertice[prev],cen))<=0 and index[prev]<j and index[prev]>start))):
        ...
        if(pos!=i and ((cross(vec(arr[pos],cen),vec(vertice[pos],cen))>=0 and index[pos]>j) or (cross(vec(arr[pos],cen),vec(vertice[pos],cen))<=0 and index[pos]<j and index[pos]>start))):
